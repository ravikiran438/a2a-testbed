...
from a2a_testbed.core.types import ReportSink
...
def run(
    ...
    summary = (
        f"[bold]{result.pass_count}/{result.pass_count + result.fail_count}[/bold] "
        f"steps passed"
    )
    if result.contracts:
        c_total = result.contracts_pass_count + result.contracts_fail_count
        summary += (
            f" · [bold]{result.contracts_pass_count}/{c_total}[/bold] "
            f"contracts passed"
        )
    ...
    sinks_raw = getattr(result, "_reports", None)  # populated in scenario module if present
...
def compat_cmd(
    ...
    for path in dialect_file or []:
        try:
            ...
            console.print(
                f"[red]✗ failed to load dialect file {path}:[/red] {exc}"
            )
    ...
    if dialect is not None:
        ...
        if forced is None:
            ...
            console.print(
                f"[red]✗ unknown dialect {dialect!r}[/red]; available: {available}"
            )
            raise typer.Exit(code=2)
...
def validate_cmd(
    ...
    for path in manifest_file or []:
        try:
            ...
            console.print(
                f"[red]✗ failed to load manifest file {path}:[/red] {exc}"
            )
            raise typer.Exit(code=2)
    ...
    if via_mcp:
        ...
        try:
            ...
            console.print(
                f"[yellow]! MCP delegation skipped:[/yellow] "
    ...
    for f in findings:
        if f.kind == FindingKind.DECLARED_OK:
            color = "green"; marker = "✓"; ok_count += 1
        elif f.kind == FindingKind.MANIFEST_NOT_FOUND:
            color = "yellow"; marker = "?"
        elif f.kind in {FindingKind.DECLARED_INVALID, FindingKind.PAYLOAD_MISSING}:
            color = "red"; marker = "✗"; hard_failures += 1
        else:
            color = "yellow"; marker = "!"
        console.print(
            f"  [{color}]{marker} {f.kind.value:30s}[/{color}] "
            f"[cyan]{f.extension_uri}[/cyan]"
        )
        if f.detail:
    ...
    if mcp_findings:
        ...
        for mf in mcp_findings:
            if mf.kind == _MK.PASSED:
                ...
                hard_failures += 1
            console.print(
                f"  [{color}]{marker} {mf.kind.value:25s}[/{color}] "
                f"[cyan]{mf.extension_uri}[/cyan]"
            )
            if mf.detail:
    ...
    if hard_failures > 0:
        console.print(
            f"[red]✗ {hard_failures} broken finding(s); see above[/red]"
        )
...
def card_cmd(
    ...
    async def _run() -> int:
        ...
        if ok_count == total:
            console.print(
                f"[green]✓ {ok_count}/{total} extensions validated[/green]"
            )
            return 0
        console.print(
...
def manifest_generate_cmd(
    ...
    name: str = typer.Option(
        ..., "--name", "-n", help="Human-readable protocol name."
    ),
    version: str = typer.Option(
        ..., "--version", "-v", help="Protocol semver (e.g. '1.0.0')."
    ),
...
def manifest_spec_cmd(
    ...
    if check:
        if output is None:
            console.print(
                "[red]✗ --check requires --output to point at the "
                "committed SPEC.md[/red]"
            )
            raise typer.Exit(code=2)
        if not output.exists():
            console.print(
                f"[red]✗ --check target does not exist: {output}[/red]"
            )
            raise typer.Exit(code=1)
        existing = output.read_text(encoding="utf-8")
        if existing != rendered:
            console.print(
                f"[red]✗ SPEC.md drift detected at {output}[/red]"
            )
            console.print(
                "[dim]Re-run without --check to regenerate.[/dim]"
            )
            raise typer.Exit(code=1)
...
def contracts_cmd(
    ...
    async def _run() -> int:
        result = await run_scenario_file(scenario, log_level=log_level)
        ...
        for decl in scenario_obj.agents:
            card_path = (
                scenario_dir / decl.card if not Path(decl.card).is_absolute()
                else Path(decl.card)
            )
            card = load_agent_card_from_path(card_path)
            runtime = PythonInProcRuntime(
                decl.id, card, scripts=dict(scripts.get(decl.id, {}))
            )
            net.register(runtime)

        async with net:
            agent_table = Table(
                title="Transport contracts (per agent)", show_lines=False
            )
            ...
            for agent_id in sorted(net._runtimes.keys()):
                ...
                for r in results:
                    ...
                    check = "[green]✓[/green]" if r.passed else "[red]✗[/red]"
                    agent_table.add_row(
                        agent_id, r.contract_id, check, r.detail or "—"
                    )
        ...
        console.print(
            f"[red]✗ {summary['failed']} of {summary['total']} contracts failed[/red]"
        )
        return 1
...
def conformance_cmd(
    ...
    async def _run() -> int:
        try:
            ...
            console.print(
                f"[red]✗ contract sweep failed before completing: {exc}[/red]"
            )
            return 2
        ...
        if failed:
            console.print(
                f"[red]✗ {failed} of {len(results)} contracts failed[/red]"
            )
...
def coverage_cmd() -> None:
    ...
    for category_dir in sorted(p for p in contracts_root.iterdir() if p.is_dir()):
        files = {
            f.stem for f in category_dir.glob("*.py")
    ...
    ROADMAP: list[tuple[str, int, int, str]] = [
        ("agentcard structural",
            impl_by_cat.get("agentcard structural", 0), 0,
            "§4.4, §8.1, §8.2, §8.3"),
        ("versioning",
            impl_by_cat.get("versioning", 0), 2,  # ~3 total - 1 done
            "§3.6"),
        ("authorization / security",
            impl_by_cat.get("authorization / security", 0), 1,  # ~4 total - 3 done
            "§7, §13"),
        ("json serialization",
            impl_by_cat.get("json serialization", 0), 0,
            "§5.5, §5.6.1"),
        ("jsonrpc envelope/errors",
            impl_by_cat.get("jsonrpc envelope/errors", 0), 0,
            "§3.1.1, §9.3, §9.5"),
        ("capability validation",
            impl_by_cat.get("capability validation", 0), 2,  # ~5 - 3 done
            "§3.3.4, §3.1.2, §3.1.7, §3.5"),
        ("extensions (spec-section)",
            impl_by_cat.get("extensions (spec-section)", 0), 4,  # ~6 - 2 done
            "§4.4.4, §4.6"),
        ("network (original)",
            impl_by_cat.get("network (original)", 0), 0,
            "(testbed-original)"),
        ("task lifecycle",
            impl_by_cat.get("task lifecycle", 0), 2,  # ~7 - 5 done
            "§3.4, §4.1.1, §4.1.3"),
        ("GetTask / ListTasks / CancelTask",
            impl_by_cat.get("GetTask / ListTasks / CancelTask", 0), 5,  # ~10 - 5 done
            "§3.1.3–§3.1.5"),
        ("multi-turn (contextId/taskId)",
            impl_by_cat.get("multi-turn (contextId/taskId)", 0), 4,  # ~5 - 1 done
            "§3.4.1–§3.4.3"),
        ("streaming (SSE)",
            impl_by_cat.get("streaming (SSE)", 0), 0,  # ~7 - 7 done
            "§3.1.2, §4.1.6, §4.1.7"),
        ("subscribe to task",
            impl_by_cat.get("subscribe to task", 0), 0,  # ~4 - 4 done
            "§3.1.6"),
        ("push notifications",
            impl_by_cat.get("push notifications", 0), 0,  # ~7 - 7 done
            "§3.1.7–§3.1.10, §3.5"),
    ]
    ...
    pct = (
        (implemented_total / overall_total * 100.0) if overall_total else 0.0
    )
    ...
    if meta.commit:
        console.print(
            f"[dim]Spec text:[/dim] [link]{meta.specification_url}[/link]"
        )
